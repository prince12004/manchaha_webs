...
.remov-variant {
    ...
    color: #FFFFFF;
    box-shadow: 0px 8px 20px #ffcccc;
    font-size: 14px;
    padding: 8px 20px;
}
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="section container mb-3">
                    ...
                    <select id="product-category" name="category" class="form-select" required>
                        ...
                        <option value="<?= $categories['CategoryID']?>"
                            <?= isset($categories['CategoryName']) && $categories['CategoryName'] == $product['CategoryName'] ? 'selected' : '' ?>>
                    ...
                    <select id="sub-category-01" name="sub-category-1" class="form-select" required>
                        ...
                        <?php foreach ($product['subcategories'] as $subcategories) {?>
                        <option value="<?= $subcategories['CategoryID']?>"
                            <?= isset($subcategories['CategoryID']) && $subcategories['CategoryID'] == $product['subcategory_id'] ? 'selected' : '' ?>>
                ...
                <div class="section container mb-3">
                    ...
                    <input id="Product-ID" name="product_id" value="<?= htmlspecialchars($product['jwellary_id']) ?>"
                    ...
                    <select id="Visibility-status" name="visibility" class="form-select" required>
                        <option value="Published"
                            <?= isset($product['visibility']) && $product['visibility'] == "Published" ? 'selected' : '' ?>>
                            Published</option>
                        <option value="Un-Published"
                            <?= isset($product['visibility']) && $product['visibility'] == "Un-Published" ? 'selected' : '' ?>>
                            Un-Published</option>
                    </select>

                    <!-- SKU -->
                    <label for="SKU" class="form-label">SKU</label>
                    <input id="SKU" type="text" name="sku" value="<?= htmlspecialchars($product['sku_code']) ?>"
                    ...
                    <select id="Product-tags" name="tag" class="form-select" required>
                        <option value="tag1"
                            <?= isset($product['tag']) && $product['tag'] == "tag1" ? 'selected' : '' ?>>Tag 01</option>
                        <option value="tag2"
                            <?= isset($product['tag']) && $product['tag'] == "tag2" ? 'selected' : '' ?>>Tag 02</option>
                    </select>

                    <!-- Publish Schedule -->
                    <label for="Publish-Schedule" class="form-label">Publish Schedule(Date and Time)</label><br>
                    <input type="text" name="publish_date" id="Publish-Schedule"
                        value="<?= date('Y-m-d\  H:i', strtotime($product['publish_at'])) ?>" class="form-input"
                        required>
                </div>

                <div class="d-flex updates-thumbnil justify-content-around flex-wrap w-100">
                    <div class="d-flex flex-column gap-2 align-items-center">
                        <?php if (isset($product['thumbnail']) && !empty($product['thumbnail'])) { ?>
                            <img style="height:75px; border-radius:10px;" src="<?= base_url('/uploads/products/') . $product['thumbnail'] ?>" alt="Thumbnail">
                        <?php } ?>
                        <button style="font-size:12px;" class="btn btn-outline-dark" type="button"
                            onclick="triggerFileInput('fileInput_<?= $product['id'] ?>')">Update Thumbnail</button>
                        <input type="file" id="fileInput_<?= $product['id'] ?>" style="display:none;" accept="image/*"
                            onchange="updateThumbnail(this, '<?= $product['id'] ?>')">
                    </div>
            ...
            <div class="col-md-8">
                <div style="max-width:900px" class="section container  mb-3">
                    ...
                    <input type="text" id="product-name" name="product_name" class="form-control"
                        value="<?= htmlspecialchars($product['jwellary_name']) ?>"
                    ...
                    <textarea id="product-description" name="product_description" class="form-control" rows="3"
                        ...
                        required><?= htmlspecialchars($product['jwellary_description']) ?></textarea>
                </div>
                <div style="max-width:900px" class="section container mb-3">
                    <div class="product-discription-cont">
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <input id="material-type" name="material_type"
                                    value="<?= htmlspecialchars($product['material_type']) ?>" type="text"
                                    class="form-input" placeholder="Crystal" required>
                            </div>
                            <div class="col-6">
                                <label for="metal Type" class="form-label">Metal Type</label>
                                <input id="metal-Type" name="metal_type"
                                    value="<?= htmlspecialchars($product['metal_type']) ?>" type="text"
                                    class="form-input" placeholder="Silver" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6">
                                <label for="Gem Type" class="form-label">Gem Type</label>
                                <input id="gem-Type" name="gem_type"
                                    value="<?= htmlspecialchars($product['gem_type']) ?>" type="text" class="form-input"
                                    placeholder="Crystal" required>
                            </div>
                            <div class="col-6">
                                <label for="Occasion Type" class="form-label">Occasion Type</label>
                                <input id="Occasion-Type" value="<?= htmlspecialchars($product['occation_type']) ?>"
                                    name="occation_type" type="text" class="form-input"
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <input id="Ideal-For" name="ideal_for" type="text"
                                    value="<?= htmlspecialchars($product['ideal_for']) ?>" class="form-input"
                                    placeholder="Women" required>
                            </div>
                            <div class="col-6">
                                <label for="Sizing" class="form-label">Sizing </label>
                                <input id="Product-Sizing" name="sizing"
                                    value="<?= htmlspecialchars($product['sizing']) ?>" type="text" class="form-input"
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <input id="product-Finish" name="product_finish"
                                    value="<?= htmlspecialchars($product['finish']) ?>" type="text" class="form-input"
                                    placeholder="Glossy" required>
                            </div>
                            <div class="col-6">
                                <label for="Generic Name" class="form-label">Generic Name</label>
                                <input id="Generic-Name" name="generic_name"
                                    value="<?= htmlspecialchars($product['generic_name']) ?>" type="text"
                                    class="form-input" placeholder="Bracelets" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 ">
                            ...
                            <select id="product-grade" name="policy_type" class="form-select">
                                <option value="return" <?= ($product['policy']==='return') ? 'selected' : '' ?>>Return
                                </option>
                                <option value="replacement"
                                    <?= ($product['policy']==='replacement') ? 'selected' : '' ?>>Replacement</option>
                                <option value="refund" <?= ($product['policy']==='refund') ? 'selected' : '' ?>>Refund
                                </option>
                            </select>
                        </div>
                        <div class="col-md-6 ">
                            <label for="product grade" class="form-label mt-3">Number of days</label>
                            <input type="number" class="form-input" name="policy_time"
                                value="<?= htmlspecialchars($product['policy_time']) ?>" placeholder="number of days">
                        </div>
                    </div>
                </div>
                            ...
                            <div class="d-flex flex-column align-items-center mb-3">
                                ...
                                <div class="d-flex justify-content-around flex-wrap w-100">
                                    ...
                                    <div class="d-flex flex-column gap-2 align-itmes-center">
                                        <img style="height:75px ; border-radius:10px;"
                                            src="<?= base_url('uploads/products/').$imag['image']?>">
                                        <button style="font-size:12px;" class="btn btn-outline-dark" type="button"
                                            onclick="updateImage('<?= $imag['image_id'] ?>')">update</button>
                                            <button style="font-size:12px;" class="btn btn-outline-dark" type="button"
                                            onclick="deleteImage('<?= $imag['image_id'] ?>')">Delete Image</button>
                                    </div>
                                    <?php } ?>
                                </div>
                                <!-- Details Section -->
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="text" name="color[<?= $variantIndex ?>]" class="form-input"
                                                value="<?= $varient['color'] ?>" placeholder="Rose Gold" required>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="Weight" class="form-label">Weight:</label>
                                            <input type="text" name="weight[<?= $variantIndex ?>]" class="form-input"
                                                value="<?= $varient['weight'] ?>" placeholder="20 g" required>
                                        </div>


                                        <div class="col-3">
                                            <label for="SKU" class="form-label">*SKU Code</label>
                                            <input type="text" name="varient_sku[<?= $variantIndex ?>]"
                                                value="<?= isset($varient['varient_sku']) ? $varient['varient_sku'] : '' ?>"
                                                class="form-input" placeholder="SKU" required>
                                        </div>
                                        <div class="col-3">
                                            <label for="hsn" class="form-label">*HSN Number</label>
                                            <input type="text" name="hsn[<?= $variantIndex ?>]"
                                                value="<?= isset($varient['hsn']) ? $varient['hsn'] : '' ?>"
                                                class="form-input" placeholder="HSN" required>
                                        </div>

                                        <div class="col-md-2">
                                            <label for="Stock" class="form-label">Stock</label>
                                            <input type="text" name="stock[<?= $variantIndex ?>]" class="form-input"
                                                value="<?= $varient['stock'] ?>" placeholder="256" required>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-4">
                                            <label for="Product Base Price" class="form-label">Product MRP
                                                :</label>
                                            <input type="text" name="base_price[<?= $variantIndex ?>]"
                                                class="form-input" value="<?= $varient['base_price'] ?>"
                                                placeholder="Type MRP" required>
                                        </div>
                                        <div class="col-4">
                                            <label for="Sale Price" class="form-label">Sale Price:</label>
                                            <input type="text" name="sale_price[<?= $variantIndex ?>]"
                                                class="form-input"
                                        ...
                                        <div class="col-4">
                                            ...
                                            <select class="form-select" name="status[<?= $variantIndex?>]">
                                                <option value="1"
                                                    <?= isset($varient['status']) && $varient['status'] == 1 ? 'selected' : '' ?>>
                                                    Active</option>
                                                <option value="2"
                                                    <?= isset($varient['status']) && $varient['status'] == 2 ? 'selected' : '' ?>>
                                                    Not-Active</option>
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="d-flex w-100 justify-content-between align-items-center">
                                    <div class="d-flex flex-column justify-content-center align-items-center mt-4 mb-2">
                                        <div id="imageContainer" style="margin-bottom: 20px;">
                                            <button
                                                style="font-size: 13px; border-radius: 25px !important; box-shadow: 0px 8px 20px #4aba4ab3;"
                                                class="btn btn-success me-3" type="button"
                                                onclick="addNewImage('<?= $varient['varient_id']?>')">Add New
                                                Image</button>

                                        </div>
                                        <div> <button class="delete-btn remov-variant" type="button">
                                                <span class="new"
                                                    onclick="deleteVarient('<?= $varient['varient_id']?>')">Delete
                                                    Varient</span>
                                            </button>
                                    ...
                                    <div class="d-flex justify-content-end mt-4">
                                        <button
                                            style="font-size:13px; border-radius:25px !important; box-shadow:0px 8px 20px #4A90E2;"
                                            class="btn btn-primary me-3" type="button"
                                            onclick="Updatevarient('<?= $variantIndex ?>', '<?= $varient['varient_id'] ?>')">Update
                ...
                <div class="w-100 d-flex justify-content-center mt-3 mb-5">
                    <button style="border-radius:25px; padding:8px 20px; box-shadow:0px 8px 20px rgba(0,0,0,0.2);"
                        class="btn btn-dark" type="button" onclick="updateProductDetails('<?= $product['id']?>')">Save
...
<div id="successModal-thumbnail" class="modal" style="display: none;">
    <div class="modal-content">
        <div class="modal-logo">
            <img src="<?= base_url('/assets/images/images/header-new-logo.png') ?>" alt="Logo">
        </div>
        <p>Thumbnail updated successfully!</p>
        <div class="d-flex justify-content-center">
            <button class="closeSuccessModal modal-btn close-btn">OK</button>
        </div>
    </div>
...
function updateProductDetails(jwellaryId) {
    console.log(document.querySelector("select[name='sub-category-1']").value);
    // Collect form data
    const formData = {
        ...
        policy: document.querySelector("select[name='policy_type']").value,
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            ...
            document.getElementById('alertResponse').innerHTML =
                "An error occurred while updating the product details";
            alertModal()
        }
    });
}
...
function Updatevarient(variantIndex, variantId) {
    ...
    const formData = {
        ...
        varient_sku: document.querySelector(`input[name='varient_sku[${variantIndex}]']`).value,
        hsn: document.querySelector(`input[name='hsn[${variantIndex}]']`).value,
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (responseData.status === 'success') {
                ...
                document.getElementById('alertResponse').innerHTML = "Failed to update variant: " +
...
$('#add-variant-btn').on('click', function(event) {
    ...
    if (variantExists) {
        ...
        document.getElementById('alertResponse').innerHTML =
            "You can only create one variant at a time. Delete the existing one to add a new variant.";
        alertModal()
        ...
        <div class="d-flex align-items-center mb-3">
            ...
            <div class="details">
                <div class="row d-flex flex-wrap">
                    ...
                    <div class="col-3">
                        <label for="hsn" class="form-label">*HSN Number</label>
                        <input type="text" name="hsn" class="form-input" placeholder="HSN" required>
                    </div>
                    <div class="col-2">
                        <label for="Weight" class="form-label">Weight:</label>
                    ...
                    <div class="col-5">
                        <label for="Product MRP" class="form-label">*Product MRP :</label>
                        <input type="text" name="base_price" class="form-input numbers-only" placeholder="Type MRP" required>
                    </div>
                    <div class="col-5">
                        <label for="Sale Price" class="form-label">*Sale Price:</label>
                        <input type="text" name="sale_price" class="form-input numbers-only" placeholder="Type sale price" required>
                        <p id="price-error" style="color: red; display: none;"><small>Product MRP should be less than or equal to the Sale Price.</small></p>
                    </div>
                    <div class="col-4">
                        <label for="Product Status" class="form-label">Product Status:</label>
...
document.addEventListener('change', function(event) {
    if (event.target.classList.contains('preview-input')) {
        ...
        const previewContainer = input.closest('.product-container-3').querySelector(
        '.image-preview-container');
            ...
            fetch('<?= base_url('addNewImage') ?>', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('alertResponse').innerHTML = "Image update failed: " + data
                            .message;
                        alertModal();
                    }
                })
...
function deleteVarient(id) {
    // Show the updated confirmation modal
    $('#deleteConfirmationModal').show();

    // When 'Yes' is clicked, proceed with deletion
    $('#confirmDelete').off('click').on('click', function() {
        // Send AJAX request to delete the variant
        $.ajax({
            url: '<?= base_url('deleteVarient') ?>',
            type: 'POST',
            data: JSON.stringify({
                id: id
            }),
            ...
            success: function(response) {
                ...
                if (resp.status === 'success') {
                    showModal('Success', 'Variant deleted successfully.');
                    setTimeout(function() {
                        window.location.reload(); // Reload the page after success
                    }, 2000); // Delay before reloading
                } else {
                    showModal('Error', 'Unable to delete variant. Please try again.');
                }
            },
            error: function(xhr, status, error) {
                showModal('Error', 'An error occurred while processing your request.');
            }
        });
    ...
    // When 'No' is clicked, close the confirmation modal
    $('#cancelDelete').off('click').on('click', function() {
        $('#deleteConfirmationModal').hide(); // Simply hide the modal
    });
}

// Function to show the success/error message modal
...
$('#closeModal').on('click', function() {
    $('#messageModal').hide(); // Hide the message modal
});
...
function deleteImage(imageId){
    ...
    $.ajax({
        url: '<?= base_url("deletVarientImage") ?>',
        type: 'POST',
...
function triggerFileInput(inputId) {
    document.getElementById(inputId).click(); // Trigger the hidden file input
}

function updateThumbnail(input, productId) {
