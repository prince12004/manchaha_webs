<?php $this->load->View('Admin/header') ?>
    ...
    $('.circle').each(function() {
        ...
        circle.circleProgress({
            ...
            fill: { gradient: ['#FE6BBA'] }, // Adjust fill colors for each circle
    ...
    $('.circle-2').each(function() {
        ...
        circle.circleProgress({
            value: percent / 100,
            size: 100, // Adjust size as needed
            fill: { gradient: ['#9267F2'] }, // Adjust fill colors for each circle
            startAngle: -Math.PI / 2, // Start from the top
             // Adjust empty fill color
        }).on('circle-animation-progress', function(event, progress) {
            circle.find('.circle-inner').html(Math.round(percent * progress) + '<i>%</i>');
    ...
    $('.circle-3').each(function() {
        ...
        circle.circleProgress({
            value: percent / 100,
            size: 100, // Adjust size as needed
            fill: { gradient: ['#76ACFE'] }, // Adjust fill colors for each circle
            startAngle: -Math.PI / 2, // Start from the top
             // Adjust empty fill color
        }).on('circle-animation-progress', function(event, progress) {
            circle.find('.circle-inner').html(Math.round(percent * progress) + '<i>%</i>');
